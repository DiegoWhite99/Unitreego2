...
async def saludo(pub_sub):
    """
    Alias de saludo simple para compatibilidad con scripts previos.
    """
...
async def main():
    ...
    # Compatibilidad entre variantes del cliente WebRTC.
    pub_sub = getattr(getattr(conn, "datachannel", None), "pub_sub", None)
    if pub_sub is None:
